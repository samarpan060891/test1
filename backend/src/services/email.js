...
async function sendEmail({ to, subject, html }) {
  ...
  try {
    const { data, error } = await client.emails.send({
      from: process.env.EMAIL_FROM || 'QC Inspection Portal <onboarding@resend.dev>',
      to: recipients,
      ...(process.env.EMAIL_CC ? { cc: process.env.EMAIL_CC.split(',').map(s => s.trim()) } : {}),
...
function emailWhDeviationRequested({ poNo, itemName, supplierName, stage, requesterName, reason, inspectionId }) {
  const url = inspectionId ? `${APP_URL}/warehouse-inspections/${inspectionId}` : `${APP_URL}/warehouse-inspections`;
  return {
    subject: `Deviation Approval Requested — ${itemName || poNo} | ${stage || ''}`,
    html: layout('Deviation Approval Requested by QA', `
      <p style="color:#475569;font-size:14px;margin:0 0 16px;">
        QA has requested a ${badge('DEVIATION', '#92400e', '#fef3c7')} approval for a warehouse inspection.
...
function emailWhDeviationDecided({ decision, poNo, itemName, supplierName, stage, buyerName, deviationReason, remarks, inspectionId }) {
  ...
  return {
    subject: `Deviation ${approved ? 'Approved' : 'Rejected'} by Buying — ${itemName || poNo} | ${stage || ''}`,
    html: layout(`Deviation ${approved ? 'Approved' : 'Rejected'} by Buying`, `
      <p style="color:#475569;font-size:14px;margin:0 0 16px;">
        The deviation request has been ${statusBadge} by Buying. Please make the <strong>final QA decision</strong> on this inspection.
      </p>
      ${jobInfoTable([
        ['PO Number', poNo || '—'],
        ['Item', itemName || '—'],
